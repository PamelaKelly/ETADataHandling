...
class database_setup():
    ...
    def insert_routes(self, route_info, distances):
        db = database_setup("password.txt", "eta.cb0ofqejduea.eu-west-1.rds.amazonaws.com", "3306", "eta", "eta")
        engine = db.db_engine()
        Session = sessionmaker(bind=engine)
        session = Session()
        ...
        for route in routes_json:
            ...
            for stop in stops:
                distance = dists[str(route)][str(stop)]
                try:
                    count_stops += 1
                    route_obj = Route(journey_pattern=route,
                                  stop_id=stop, distance_from_terminus=distance)
